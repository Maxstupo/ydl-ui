...
namespace Maxstupo.YdlUi.Util {
    ...
    // Extracted from ByteSize library - https://github.com/omar/ByteSize
    // Modified by Maxstupo - "From(double, FilesizeUnit)"
    /// <summary>
    ...
    public struct ByteSize : IComparable<ByteSize>, IEquatable<ByteSize> {
        ...
        public static ByteSize From(double value, FilesizeUnit unit) {
            switch (unit) {
                case FilesizeUnit.KB: return FromKiloBytes(value);
                case FilesizeUnit.MB: return FromMegaBytes(value);
                case FilesizeUnit.GB: return FromGigaBytes(value);
                default: break;
            }
            throw new Exception();
        }
